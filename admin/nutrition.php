...
if (isset($_GET['delete']) && !isset($_GET['delete_done'])) {
    ...
    if ($stmt->execute()) {
        ...
        header("Location: nutrition.php?delete_done=success");
        exit();
    } else {
        // Redirect back to the same page with an error flag
        header("Location: nutrition.php?delete_done=error");
        exit();
    }
                                ...
                                <?php while ($row = $result->fetch_assoc()): ?>
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <a href="#" class="btn btn-sm btn-danger" onclick="showDeleteModal(<?php echo $row['nutrition_id']; ?>)">Delete</a>
                                        </td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
...
<!--sript to delete the modal-->
<script>
    let deleteUrl = '';

    function showDeleteModal(nutritionId) {
        deleteUrl = '?delete=' + nutritionId;
